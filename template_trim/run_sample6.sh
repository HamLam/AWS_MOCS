...
# else
   # CREATE DATABASE
 #   mysql --socket=$BASE/thesock -u root -e "CREATE DATABASE cnv6;"
     
  #   echo "Load tables"
    # find $table_path -name "*.sql" | xargs -I {} -n 1 -P 24 sh -c "mysql --socket=$BASE/thesock -u root cnv6 < \"{}\" "
  
  
  # echo "mysql --socket=$BASE/thesock -u root cnv6 < $table_path/training_data_2016_07.sql" >> $working_dir/basetables_load_commands
  # echo "mysql --socket=$BASE/thesock -u root cnv6 < $table_path/tso_data.sql" >> $working_dir/basetables_load_commands
  # echo "mysql --socket=$BASE/thesock -u root cnv6 < $table_path/tso_exon_60bp_segments_main_data.sql" >> $working_dir/basetables_load_commands
  # echo "mysql --socket=$BASE/thesock -u root cnv6 < $table_path/tso_exon_60bp_segments_pileup.sql" >> $working_dir/basetables_load_commands
  # echo "mysql --socket=$BASE/thesock -u root cnv6 < $table_path/tso_exon_60bp_segments_window_data.sql">> $working_dir/basetables_load_commands
  # echo "mysql --socket=$BASE/thesock -u root cnv6 < $table_path/tso_exon_contig_pileup.sql" >> $working_dir/basetables_load_commands
  # echo "mysql --socket=$BASE/thesock -u root cnv6 < $table_path/tso_reference_exon.sql" >> $working_dir/basetables_load_commands
  # echo "mysql --socket=$BASE/thesock -u root cnv6 < $table_path/tso_reference.sql" >> $working_dir/basetables_load_commands
  # echo "mysql --socket=$BASE/thesock -u root cnv6 < $table_path/tso_windows_padded_pileup.sql" >> $working_dir/basetables_load_commands
...
echo -n "Skipped Finished Mysql load base tables cnv6 " >> $working_dir/time_check
timecheck=`(date +"%Y-%m-%d [ %H:%M:%S ]")`;
echo ${timecheck} >> $working_dir/time_check
...
if  [ "$?" = "0" ]; then
 ...
 echo "Run Pre_Load_Control.sql"
    mysql --socket=$BASE/thesock -u root cnv6 < Pre_Load_Control.sql
    if [[ $? -ne 0 ]] ; then
...
grep "Pre_Load_Sample.sql" $working_dir/completed.txt > /dev/null 2>&1
 ...
 echo "Run Pre_Load_Sample.sql"
    mysql --socket=$BASE/thesock -u root cnv6 < Pre_Load_Sample.sql
     if [[ $? -ne 0 ]] ; then
...
mysqlimport --local --socket=$BASE/thesock -u root cnv6 --use-threads=4 cnv_control_name_bwa_pileup_no_dup.chr*_t cnv_control_name_bwa_pileup.chr*_t cnv_control_name_bowtie_pileup.chr*_t cnv_sample_name_bwa_pileup_no_dup.chr*_t cnv_sample_name_bwa_pileup.chr*_t cnv_sample_name_bowtie_pileup.chr*_t
    # mysqlimport --local --socket=$BASE/thesock -u root cnv6 --use-threads=5 --debug-check \
    # cnv_control_name_bwa_pileup_no_dup.chr17_t \
    # cnv_control_name_bwa_pileup_no_dup.chr18_t \
    # cnv_control_name_bwa_pileup_no_dup.chr19_t \
    # cnv_control_name_bwa_pileup_no_dup.chr20_t \
    ...
    # cnv_control_name_bwa_pileup.chr18_t \
    # cnv_control_name_bwa_pileup.chr19_t \
    # cnv_control_name_bwa_pileup.chr20_t \
    ...
    # cnv_control_name_bowtie_pileup.chr18_t \
    # cnv_control_name_bowtie_pileup.chr19_t \
    # cnv_control_name_bowtie_pileup.chr20_t \
...
#     cnv_sample_name_bwa_pileup_no_dup.chr17_t \
    # cnv_sample_name_bwa_pileup_no_dup.chr18_t \
    # cnv_sample_name_bwa_pileup_no_dup.chr19_t \
    # cnv_sample_name_bwa_pileup_no_dup.chr20_t \
    ...
    # cnv_sample_name_bwa_pileup.chr18_t \
    # cnv_sample_name_bwa_pileup.chr19_t \
    # cnv_sample_name_bwa_pileup.chr20_t \
    ...
    # cnv_sample_name_bowtie_pileup.chr18_t \
    # cnv_sample_name_bowtie_pileup.chr19_t \
#     cnv_sample_name_bowtie_pileup.chr20_t \
...
#     cnv_sample_name_bowtie_pileup.chr10_t \
 #    cnv_sample_name_bowtie_pileup.chr13_t \
  ...
  if [[ $? -ne 0 ]]; then
   ...
   echo "g6 Run mysqlimport failed"
       ## mysqladmin --socket=$BASE/thesock shutdown -u root
        exit 1
     else
...
if [ "$?" = "0" ]; then
    ...
    mysql --socket=$BASE/thesock -u root cnv6 < load_control.sql
    if [[ $? -ne 0 ]] ; then
...
if [ "$?" = "0" ]; then
    ...
    mysql --socket=$BASE/thesock -u root cnv6 < load_sample.sql
    if [[ $? -ne 0 ]] ; then
...
if [ "$?" = "0" ]; then
    ...
    mysql --socket=$BASE/thesock -u root cnv6 < create_reference.sql
    if [[ $? -ne 0 ]] ; then
...
if [ "$?" = "0" ]; then
    ...
    mysql --socket=$BASE/thesock -u root cnv6 < create_tables_part1.sql
    if [[ $? -ne 0 ]] ; then
...
if [ "$?" = "0" ]; then
    ...
    mysql --socket=$BASE/thesock -u root cnv6 < create_tables_ref_v1.sql
    if [[ $? -ne 0 ]] ; then
...
if [ "$?" = "0" ]; then
    ...
    mysql --socket=$BASE/thesock -u root cnv6 < create_tables_ref_v2.sql
    if [[ $? -ne 0 ]] ; then
...
if [ "$?" = "0" ]; then
    ...
    mysql --socket=$BASE/thesock -u root cnv6 < create_coverage.sql
    if [[ $? -ne 0 ]] ; then
...
if [ "$?" = "0" ]; then
    ...
    mysql --socket=$BASE/thesock -u root cnv6 < create_sample_coverage.sql
    if [[ $? -ne 0 ]] ; then
...
if [ "$?" = "0" ]; then
    ...
    mysql --socket=$BASE/thesock -u root cnv6 < create_control_coverage.sql
    if [[ $? -ne 0 ]] ; then
...
if [ "$?" = "0" ]; then
    ...
    mysql --socket=$BASE/thesock -u root cnv6 < cnv_tables.sql
    if [[ $? -ne 0 ]] ; then
...
if [ "$?" = "0" ]; then
    ...
    mysql --socket=$BASE/thesock -u root cnv6 < cnv_tables_amplifications.sql
    if [[ $? -ne 0 ]] ; then
...
if [ "$?" = "0" ]; then
    ...
    mysql --socket=$BASE/thesock -u root cnv6 < ordered_genes.sql
    if [[ $? -ne 0 ]] ; then
...
if [ "$?" = "0" ]; then
    ...
    mysql --socket=$BASE/thesock -u root cnv6 < create_data.sql
    if [[ $? -ne 0 ]] ; then
...
if [ "$?" = "0" ]; then
    ...
    mysql --socket=$BASE/thesock -u root cnv6 < get_machine_learning_data.sql > sample_name_raw_data_$_now.txt
    if [[ $? -ne 0 ]] ; then
...
if [ "$?" = "0" ]; then
    ...
    mysql --socket=$BASE/thesock -u root cnv6 < combine_data.sql
    if [[ $? -ne 0 ]] ; then
...
if [ "$?" = "0" ]; then
    ...
    mysql --socket=$BASE/thesock -u root cnv6 < get_predicted.sql > sample_name_predicted_$_now.txt
    if [[ $? -ne 0 ]] ; then
...
if [ "$?" = "0" ]; then
    ...
    mysql --socket=$BASE/thesock -u root cnv6 < get_ordered_genes.sql  > sample_name_cnv_calls_on_ordered_genes_$_now.txt
    if [[ $? -ne 0 ]] ; then
...
if [ "$?" = "0" ]; then
    ...
    mysql --socket=$BASE/thesock -u root cnv6 < get_qc_data.sql > sample_name_qc_data_$_now.txt
    if [[ $? -ne 0 ]] ; then
