...
grep "mysqlimport" $working_dir/completed.txt > /dev/null 2>&1
 ...
 echo "Run mysqlimport"
     ...
     mysqlimport --local --socket=$BASE/thesock -u root cnv2 --use-threads=8 \
...
if [ "$?" = "0" ]; then
	...
	exit 1
        ...
        echo "sample 2 create_tables_part1.sql done"
...
if [ "$?" = "0" ]; then
	...
	exit 1
        ...
        echo "sample 2 normalize_coverage.R done"
...
if [ "$?" = "0" ]; then
	...
	exit 1
        ...
        echo "sample 2 smooth_coverage.R done"
...
if [ "$?" = "0" ]; then
	...
	exit 1
        ...
        echo "sample 2 get_three_ref.R done"
...
if [ "$?" = "0" ]; then
	...
	exit 1
        ...
        echo "sample 2 create_tables_ref_v1.sql done"
...
if [ "$?" = "0" ]; then
	...
	exit 1
     ...
     echo "sample 2 create_tables_ref.R done"
...
if [ "$?" = "0" ]; then
	...
	exit 1
       ...
       echo "sample 2 create_tables_ref_v2.sql done"
...
if [ "$?" = "0" ]; then
	...
	exit 1
        ...
        echo "sample 2 create_coverage.sql done"
...
if [ "$?" = "0" ]; then
	...
	exit 1
        ...
        echo "sample 2 create_sample_coverage.sql done"
...
if [ "$?" = "0" ]; then
	...
	exit 1
       ...
       echo "sample 2 create_control_coverage.sql done"
...
if [ "$?" = "0" ]; then
	...
	exit 1
        ...
        echo "sample 2 cnv_tables.sql done"
...
if [ "$?" = "0" ]; then
	...
	exit 1
       ...
       echo "sample 2 cnv_tables_amplifications.sql done"
...
#else
#    echo "plot_script.pl"
#    perl $script_path/plot_script.pl -t cnv_sample_name_tso_over_control_name_n_bowtie_bwa_ratio_gene_out -s sample_name -c cnv_sample_name_ordered_genes -k cnv_sample_name_ordered_genes -h localhost -u root -d cnv2 -o plot_genes_ordered.py -ms $mysql_socket -a 1
#    if [[ $? -ne 0 ]] ; then
