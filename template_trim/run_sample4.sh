...
if  [ "$?" = "0" ]; then
 ...
 echo "Run Pre_Load_Control.sql"
    mysql --socket=$BASE/thesock -u root cnv4 < Pre_Load_Control.sql
    if [[ $? -ne 0 ]] ; then
...
grep "Pre_Load_Sample.sql" $working_dir/completed.txt > /dev/null 2>&1
 ...
 echo "Run Pre_Load_Sample.sql"
    mysql --socket=$BASE/thesock -u root cnv4 < Pre_Load_Sample.sql
     if [[ $? -ne 0 ]] ; then
...
grep "mysqlimport" $working_dir/completed.txt > /dev/null 2>&1
 ...
 echo "Run mysqlimport"
     ...
     mysqlimport --local --socket=$BASE/thesock -u root cnv4 --use-threads=5 --debug-check \
     cnv_control_name_bwa_pileup_no_dup.chr9_t \
     cnv_control_name_bwa_pileup_no_dup.chr10_t \
     cnv_control_name_bwa_pileup_no_dup.chr11_t \
     cnv_control_name_bwa_pileup_no_dup.chr12_t \
     ...
     cnv_control_name_bwa_pileup.chr11_t \
     cnv_control_name_bwa_pileup.chr12_t \
     ...
     cnv_control_name_bowtie_pileup.chr11_t \
     cnv_control_name_bowtie_pileup.chr12_t \
     ...
     cnv_sample_name_bwa_pileup_no_dup.chr11_t \
     cnv_sample_name_bwa_pileup_no_dup.chr12_t \
     ...
     cnv_sample_name_bwa_pileup.chr11_t \
     cnv_sample_name_bwa_pileup.chr12_t \
     ...
     cnv_sample_name_bowtie_pileup.chr11_t \
     cnv_sample_name_bowtie_pileup.chr12_t \
  ...
  if [[ $? -ne 0 ]]; then
   echo "Run mysqlimport failed" >&2
       ## mysqladmin --socket=$BASE/thesock shutdown -u root
   echo "g4 Run mysqlimport failed"
        exit 1
     else
...
if [ "$?" = "0" ]; then
    ...
    mysql --socket=$BASE/thesock -u root cnv4 < load_control.sql
    if [[ $? -ne 0 ]] ; then
...
if [ "$?" = "0" ]; then
    ...
    mysql --socket=$BASE/thesock -u root cnv4 < load_sample.sql
    if [[ $? -ne 0 ]] ; then
...
if [ "$?" = "0" ]; then
    ...
    mysql --socket=$BASE/thesock -u root cnv4 < create_reference.sql
    if [[ $? -ne 0 ]] ; then
...
if [ "$?" = "0" ]; then
    ...
    mysql --socket=$BASE/thesock -u root cnv4 < create_tables_part1.sql
    if [[ $? -ne 0 ]] ; then
...
if [ "$?" = "0" ]; then
    ...
    mysql --socket=$BASE/thesock -u root cnv4 < create_tables_ref_v1.sql
    if [[ $? -ne 0 ]] ; then
...
if [ "$?" = "0" ]; then
    ...
    mysql --socket=$BASE/thesock -u root cnv4 < create_tables_ref_v2.sql
    if [[ $? -ne 0 ]] ; then
...
if [ "$?" = "0" ]; then
    ...
    mysql --socket=$BASE/thesock -u root cnv4 < create_coverage.sql
    if [[ $? -ne 0 ]] ; then
...
if [ "$?" = "0" ]; then
    ...
    mysql --socket=$BASE/thesock -u root cnv4 < create_sample_coverage.sql
    if [[ $? -ne 0 ]] ; then
...
if [ "$?" = "0" ]; then
    ...
    mysql --socket=$BASE/thesock -u root cnv4 < create_control_coverage.sql
    if [[ $? -ne 0 ]] ; then
...
if [ "$?" = "0" ]; then
    ...
    mysql --socket=$BASE/thesock -u root cnv4 < cnv_tables.sql
    if [[ $? -ne 0 ]] ; then
...
if [ "$?" = "0" ]; then
    ...
    mysql --socket=$BASE/thesock -u root cnv4 < cnv_tables_amplifications.sql
    if [[ $? -ne 0 ]] ; then
...
if [ "$?" = "0" ]; then
    ...
    mysql --socket=$BASE/thesock -u root cnv4 < ordered_genes.sql
    if [[ $? -ne 0 ]] ; then
...
if [ "$?" = "0" ]; then
    ...
    mysql --socket=$BASE/thesock -u root cnv4 < create_data.sql
    if [[ $? -ne 0 ]] ; then
...
if [ "$?" = "0" ]; then
    ...
    mysql --socket=$BASE/thesock -u root cnv4 < get_machine_learning_data.sql > sample_name_raw_data_$_now.txt
    if [[ $? -ne 0 ]] ; then
...
if [ "$?" = "0" ]; then
    ...
    mysql --socket=$BASE/thesock -u root cnv4 < combine_data.sql
    if [[ $? -ne 0 ]] ; then
...
if [ "$?" = "0" ]; then
    ...
    mysql --socket=$BASE/thesock -u root cnv4 < get_predicted.sql > sample_name_predicted_$_now.txt
    if [[ $? -ne 0 ]] ; then
...
if [ "$?" = "0" ]; then
    ...
    echo "plot_script.pl"
    perl $script_path/plot_script.pl -t cnv_sample_name_tso_over_control_name_n_bowtie_bwa_ratio_gene_out -s sample_name -c cnv_sample_name_ordered_genes -k cnv_sample_name_ordered_genes -h localhost -u root -d cnv4 -o plot_genes_ordered.py -ms $mysql_socket -a 1
    if [[ $? -ne 0 ]] ; then
...
if [ "$?" = "0" ]; then
    ...
    mysql --socket=$BASE/thesock -u root cnv4 < get_ordered_genes.sql  > sample_name_cnv_calls_on_ordered_genes_$_now.txt
    if [[ $? -ne 0 ]] ; then
...
if [ "$?" = "0" ]; then
    ...
    mysql --socket=$BASE/thesock -u root cnv4 < get_qc_data.sql > sample_name_qc_data_$_now.txt
    if [[ $? -ne 0 ]] ; then
